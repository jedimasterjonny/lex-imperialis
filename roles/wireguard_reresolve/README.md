...
wg-quick's `DNS=` line runs openresolv, which creates and writes state under
...
`wg-quick up` is denied and tears the tunnel down at boot.

A local CIL policy module (`files/wireguard-reresolve.cil`, loaded with
`semodule`) grants `wireguard_t` manage on `var_run_t` — the label
`/run/resolvconf` gets by default, so wg-quick creates and manages the tree
itself — plus write on `net_conf_t` for `resolv.conf`. The grant is bounded to
the `wireguard_t` domain. (An earlier private-type variant needed the boot-time
`systemd_tmpfiles_t` domain granted too and broke at boot; `var_run_t` needs no
second domain.) The install is gated on `ansible_selinux.status == 'enabled'`, a
no-op on the SELinux-less molecule containers whose shared kernel would otherwise
take the change host-wide.
...
`default`/`leap` container tiers do not test SELinux at all.
