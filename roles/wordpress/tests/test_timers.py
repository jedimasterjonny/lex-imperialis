...
def test_cron_success_metric(host, cron_timer_stopped):
...
def test_cron_failure_metric(host, cron_timer_stopped):
...
def test_db_dump_timer_active(host):
    assert host.run("systemctl is-active wordpress-db-dump.timer").stdout.strip() == "active"


def test_db_dump_service_runs(db_dump_run):
    assert db_dump_run == 0


def test_db_dump_landed(host, db_dump_run):
...
def test_db_dump_success_metric(host, db_dump_run):
...
def test_db_dump_failure_metric(host, db_dump_run):
...
def test_update_check_timer_active(host):
    active = host.run("systemctl is-active wordpress-update-check.timer").stdout.strip()
    assert active == "active"


def test_update_check_service_runs(update_check_run):
    assert update_check_run[0] == 0


def test_update_check_metric(update_check_run):
