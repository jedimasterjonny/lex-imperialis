...
- **Container data is a named volume, not a host bind mount.** A container's mutable backing store is a podman named volume (`<role>-config`/`<role>-data`), referenced `Volume=<name>:/path` and auto-created on start — no `.volume` unit; podman labels it `container_file_t`, so no `:Z`, and `podman_backup` captures every volume a play does not exclude. Bind mounts are reserved for: read-only Ansible-rendered config (`/etc/<role>`, config-as-code) mounted `ro,Z` where the path's policy-default label is unreadable to `container_t` under enforcing SELinux (`httpd_config_t` for `/etc/caddy`) — podman relabels it `container_file_t` on each start, undoing any autorelabel — or plain `ro` where that default label is already readable (grafana's `/etc/grafana`), and `ro,z` where a second container mounts the same path (wp-cli's `--volumes-from wordpress`) — `Z` stamps the file with the private categories of whichever container mounted last and locks the other out; a read-only config file or directory mounted on top of the volume, `ro,Z` or plain `ro`; the NFS media share; and host introspection (`/`, `/sys`, the podman socket).
...
- **An image pin renders only into a quadlet unit or a `/usr/local/bin` script.** `arbites`'s container fast path applies a renovate bump by swapping the ref wherever it appears under `/etc/containers/systemd` and `/usr/local/bin` (`bin/container-swap.sh`); a pin rendered anywhere else is left stale until the next full apply. The fast path's coverage assert catches a pin found nowhere, but a pin that renders both inside and outside those paths goes stale *silently* — which is why this is a contract, not a convenience. The fast path also assumes a bump's only converge effect is that unit's restart — a role whose bump needs more will be swapped and bounced anyway, with nothing to catch it, so keep the pin's only converge effect the restart. `/usr/local/sbin` is swept as a tripwire (`bin/README.md`).
...
Nine roles carry both arms: `stow`, `common`, `podman`, `firewalld`,
`autoupdate` and `smartmon`, plus the backup chain `nfs`, `restic_backup` and
`podman_backup`, which auspex runs for its podman repo. Everything else is
single-OS and should stay that way — this is a tax, not a direction.
