...
DUMMY_TOKEN = "molecule-dummy-token-molecule-dummy-token"  # noqa: S105
PROBE = "/etc/caddy/Caddyfile.dns01-probe"


def test_health_endpoint_answers(host: Target) -> None:
    def ready() -> bool | None:
        return http_probe(host, host_header="localhost").status == HTTPStatus.NO_CONTENT or None
...
def test_wildcard_404s_unregistered_subdomain(host: Target) -> None:
    assert http_probe(host, host_header="probe." + DOMAIN).status == HTTPStatus.NOT_FOUND


# A foreign Host matches no site block; the http:// catch-all answers 404.
def test_foreign_host_404s(host: Target) -> None:
    assert http_probe(host, host_header="nonexistent.example").status == HTTPStatus.NOT_FOUND


# Renovate bumps the image unattended; catch a build that drops the DNS-01
# plugin before it reaches the fleet.
def test_cloudflare_dns_module_compiled(host: Target) -> None:
    res = host.run("podman exec caddy caddy list-modules")
    assert "dns.providers.cloudflare" in res.stdout


@pytest.fixture(scope="module")
def dns01_probe(host: Target) -> None:
    ...
    # autoescape would corrupt a Caddyfile: this renders config, not HTML.
...
def test_dns01_branch_renders_acme_dns(host: Target, dns01_probe: None) -> None:
    assert "acme_dns cloudflare" in host.run("cat " + PROBE).stdout


def test_dns01_caddyfile_valid(host: Target, dns01_probe: None) -> None:
    res = host.run(
...
def test_healthcheck_passes(host: Target) -> None:
...
def test_caddy_capabilities(host: Target) -> None:
