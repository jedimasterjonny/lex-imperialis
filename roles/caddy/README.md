...
answering 204 — which the container's own podman healthcheck probes as its restart
backstop, not as the monitor (see `roles/CLAUDE.md`); monitoring is the blackbox probe,
which reaches every proxied service through this proxy. Anything matching no site —
