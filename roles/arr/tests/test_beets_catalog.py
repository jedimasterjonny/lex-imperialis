"""The standing beets library catalog: timer, script, config, and idempotent import.
...
def test_catalog_timer_enabled_active(host: Target) -> None:
...
def test_catalog_script_present(host: Target) -> None:
    res = host.run("stat -c '%a' /usr/local/sbin/beets-library.sh")
    assert res.returncode == 0
    assert res.stdout.strip() == "755"


def test_catalog_config_no_media_writes(host: Target) -> None:
...
def test_catalog_runs_on_empty_library(host: Target) -> None:
...
def test_catalog_imports_and_is_idempotent(host: Target) -> None:
    host.run(
