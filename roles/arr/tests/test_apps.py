...
# The statuses a proxied webui may answer with before it is configured (a
# redirect to setup, or an auth challenge), any of which proves it is up.
PROXIED_OK = (
    HTTPStatus.OK,
...
def test_proxied_app_answers(host: Target, app: str) -> None:
    def ready() -> int | None:
        code = http_probe(host, host_header=app + "." + DOMAIN).status
        return code if code in PROXIED_OK else None
...
def test_hostnet_plex_answers(host: Target) -> None:
    def ready() -> bool | None:
        return http_probe(host, path="/identity", port=32400).status == HTTPStatus.OK or None

    wait_for(ready, tries=40, delay=3, fail="plex did not answer 200 on its host port")


@pytest.mark.parametrize("app", PORTLESS)
def test_portless_app_active(host: Target, app: str) -> None:
...
def test_app_account_drop(host: Target, app: str, uid: int, gid: int) -> None:
...
def test_recyclarr_account(host: Target) -> None:
...
def test_app_posture(host: Target, app: str) -> None:
...
def deployed(host: Target) -> set[str]:
...
def test_posture_covers_every_app(deployed: set[str]) -> None:
    assert set(POSTURE) == deployed


def test_reachability_covers_every_app(deployed: set[str]) -> None:
    assert set(PROXIED) | set(PORTLESS) | {"plex"} == deployed


def test_account_check_covers_every_lscr_app(host: Target, deployed: set[str]) -> None:
...
def test_healthcheck_covers_every_checked_app(host: Target, deployed: set[str]) -> None:
...
def test_app_healthcheck_passes(host: Target, app: str) -> None:
    def ready() -> bool | None:
