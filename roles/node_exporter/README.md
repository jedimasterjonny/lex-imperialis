...
for the `HELP` lines). `autoupdate`, `inquisition`, `offsite_mirror`,
`r2_mirror`, `restic_backup`, `smartmon` and `wordpress` (cron and db-dump)
consume it; `arbites` keeps its own script, as it publishes a third metric.
