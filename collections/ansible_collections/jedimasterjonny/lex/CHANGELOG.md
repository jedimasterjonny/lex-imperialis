...
Adds the nfs\_client role\, which mounts NFS shares as systemd \.mount/\.automount units \(lazy automount\) with per\-share overrides and a configurable NFS version\.

<a id="minor-changes"></a>
### Minor Changes

* nfs\_client \- new role mounting NFS shares as systemd \.mount/\.automount units\, with per\-share overrides and a configurable NFS version\.
...
Adds argument specs \(<code>meta/argument\_specs\.yml</code>\) to every role\, so inputs
are type\-validated and documented at role entry\. The dev role gains a
<code>dev\_install\_node</code> gate that installs the Node\.js runtime the Ansible MCP
server\'s <code>npx</code> launcher needs — sourcing a Node \>\= 24 from the
devel\:languages\:nodejs OBS repo on Leap\, whose default Node is too old — and
the common role adds <code>tmux</code> to the baseline package set\.

<a id="minor-changes-1"></a>
### Minor Changes

* common \- add <code>tmux</code> to the baseline package set installed on every host\.
* common \- add an argument spec \(<code>meta/argument\_specs\.yml</code>\) so role inputs are type\-validated and documented at role entry\.
* dev \- add <code>dev\_install\_node</code> \(default <code>false</code>\) and <code>dev\_node\_packages</code> to install the Node\.js runtime\, providing the <code>npx</code> launcher the Ansible MCP server runs under\.
* dev \- add an argument spec \(<code>meta/argument\_specs\.yml</code>\) so role inputs are type\-validated and documented at role entry\.
* dev \- on openSUSE Leap\, source the Node\.js runtime for <code>dev\_install\_node</code> from the <code>devel\:languages\:nodejs</code> OBS repository and pin <code>nodejs24</code>/<code>npm24</code> so the installed Node satisfies the Ansible MCP server\'s Node \>\= 24 requirement \(Leap\'s default Node is 22\, too old\)\. Tumbleweed is unchanged — its <code>npm\-default</code> already pulls Node \>\= 24\.
* motd \- add an argument spec \(<code>meta/argument\_specs\.yml</code>\) so <code>motd\_text</code> is type\-validated and documented at role entry\.
