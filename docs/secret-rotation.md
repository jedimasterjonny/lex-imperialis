...
- **`/etc/arbites/ssh/id_ed25519`** — a copy of the operator's fleet SSH key the reconcile timer connects with (it cannot be vaulted: the reconciler needs it to reach the fleet). Rotation is fleet-wide, and the public half is declared, not placed by hand: add the new key to `inventory/group_vars/all/authorized_keys.yml` and apply, re-seed the private half via the arbites bootstrap, then confirm a reconcile. It is the operator's own key, so it sits in **both** lists there — `common_authorized_keys` and `common_ansible_authorized_keys` — and rotating only the second leaves every host's owner account trusting the retired key. Two things the declared list does not cover. **Removing the old key is not yet automated:** while `common_ansible_authorized_keys_exclusive` is `false` the role only adds, so dropping the entry revokes nothing and the apply still goes green — delete the line from each host's `authorized_keys` by hand until the flag is on. And **the first-boot seeds carry it too** — `bootstrap/rogue-trader.bu` once, `bootstrap/auspex-user-data.yaml` on both of its accounts — so a rotation that skips them leaves a reflashed host trusting the retired key and rejecting the new one. auspex's documented recovery *is* a reflash, so that is not a remote case. See the role's README.
