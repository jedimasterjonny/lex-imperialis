...
| --- | --- | --- | --- |
| `caddy_cloudflare_api_token` | Cloudflare token on every zone caddy answers DNS-01 for — `jonnyoc.uk`, which holds `caddy_domain`, and `emmasedit.com` (Zone:Read, DNS:Edit) | `PLAY=solar`, then `PLAY=rogue-trader` | One token for both caddy hosts. Gates solar's DNS-01 wildcard, homepage TLS, and emmasedit.com's cert on rogue-trader. caddy serves persisted certs from the `caddy-data` volume and touches the token only at renewal, so it stays healthy for weeks on a dead one — verify at Cloudflare before revoking, not by a green container |
| `r2_reclusiam_readonly_access_key_id` / `r2_reclusiam_readonly_secret_access_key` | R2 API token, **Object Read only**, scoped to the `reclusiam` bucket — under R2 → Manage API tokens, not the Cloudflare API token page | `PLAY=auspex` | The `r2_mirror` probe's own pair, and deliberately not the Admin Read & Write one the Hyper Backup tasks hold: this host must not be able to delete the copy it watches. It only lists, so a wrong or revoked key is not silent — the probe fails closed and `R2MirrorProbeFailed` raises after two daily runs. Two variables, one token: rotate both together |
...
| `homepage_unifi_api_key` | UniFi Network API key, same issuer as `unpoller_api_key` but homepage's own — one credential per consumer | `PLAY=solar` | Same passphrase-grade caveat as `unpoller_api_key`, exposed-key WiFi rotation included. Verify on the dashboard: the UniFi tile repopulates; a refused key shows an API error on the tile while its siteMonitor dot stays green |
...
| `solar_tautulli_api_key` | Tautulli → Settings → Web Interface → API key, regenerated in place — the old value dies on save, so step 5 is already done and the widget errors until step 3 lands | `PLAY=solar` | Homepage's Tautulli widget is the only consumer; its `/status` monitor is unauthenticated and stays green. The first key, after Tautulli's first start, is read back the same way |
...
- **R2 (`reclusiam`) S3 access key, Admin Read & Write** — the credential the DSM Hyper Backup jobs authenticate to the R2 bucket with, and distinct from the read-only pair `r2_mirror` uses in the table above. Admin rather than object-scoped because Hyper Backup's target wizard lists the account's buckets before it will offer one, which an object-scoped token cannot do. It lives in each Hyper Backup task on the NAS and nowhere else — deliberately not in the vault, as the storage box credential is not: a token that can delete every object in the last-resort copy should not also be reachable through the one password that unlocks every backup. Losing it with the NAS costs minting a replacement in the Cloudflare dashboard, not the copy, and the read-only pair is enough to confirm the copy is still there while you do. Rotate under R2 → Manage API tokens, update every task that holds it, then delete the old token — generating a new one does not revoke the old. It is an S3 key pair, unrelated to the Cloudflare API tokens elsewhere in this document: it reaches this one bucket and nothing else in the account.

- **The Cloudflare WordPress plugin's purge token** (emmasedit.com) — an API token over that one zone with Zone → Cache Purge: Purge and Zone → Zone: Read, held in `wp_options` on rogue-trader as `cloudflare_api_key` (with `cloudflare_api_email` beside it) and nowhere else. Not in the vault, and not `terraform_cloudflare_api_token` reused: it exists so the plugin's "Auto Purge Content On Update" can clear the HTML that `terraform/edge-emmasedit-com.tf`'s `cache_html` rule holds at the edge for two hours, and a leak lets someone empty that cache and read zone metadata, nothing more. Rotate by minting the replacement in the Cloudflare dashboard, pasting it into wp-admin → Settings → Cloudflare after the plugin's own Log out, then deleting the old token — minting does not revoke. The daily database dump carries it into the restic repos, so a restore brings it back and a rotation needs no repo change.
