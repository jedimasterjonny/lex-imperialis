...
Fleet playbooks live in `playbooks/`; the bootstrap and molecule playbooks stay with their tooling (`bootstrap/`, `molecule/<tier>/`). Operator runbooks live in `docs/` (e.g. disaster recovery). OpenTofu config for cloud infrastructure lives in a sibling `terraform/` tree, with remote state in a GCS bucket in the infra-shared GCP project; a PR plans it and a merge to main auto-applies it to live cloud infra via CI (keyless WIF) — see `terraform/README.md`. The `jonnyoc.uk` website is a Hugo static site in `jonnyoc-site/`, built and deployed to Firebase Hosting by CI — see `jonnyoc-site/README.md`. `packer/` builds an openSUSE MicroOS image for rogue-trader in two stages against real Hetzner servers, ahead of a move off Leap that has not happened yet; the build is operator-run, never CI-run, and only its `fmt`/`validate` gates run in CI — see `packer/README.md`.
...
- **Setup**: `python -m venv .venv && . .venv/bin/activate && pip install -r requirements-dev.txt`, then `make hooks` to install the pre-commit hooks. The `terraform/` gates additionally need `tofu` and `tflint` on PATH — the `dev` role provisions both on the workstation; see `terraform/README.md` to install them elsewhere. The alert-rules gate needs `promtool` on PATH too — CI installs a pinned prometheus release, so install it from the same release locally. The `packer/` gates need HashiCorp `packer` on PATH, which the `dev` role deliberately does not install, being needed only when `packer/` changes — install it by hand, and set `PACKER` if the real binary is not first on PATH (openSUSE's cracklib owns the name); see `packer/README.md`. The `*.bu` gate needs `butane`, which the `dev` role does provide.
