...
- **Setup**: `python -m venv .venv && . .venv/bin/activate && pip install -r requirements-dev.txt`, then `make hooks` to install the pre-commit hooks. The `terraform/` gates additionally need `tofu` and `tflint` on PATH — the `dev` role provisions both on the workstation; see `terraform/README.md` to install them elsewhere. The alert-rules gate needs `promtool` on PATH too, which the `dev` role provisions from zypper; see `roles/dev/README.md`. The `packer/` gates need HashiCorp `packer` on PATH, which the `dev` role deliberately does not install, being needed only when `packer/` changes — install it by hand, and set `PACKER` if the real binary is not first on PATH (openSUSE's cracklib owns the name); see `packer/README.md`. The `*.bu` gate needs `butane`, which the `dev` role does provide.
