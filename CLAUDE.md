...
Fleet playbooks live in `playbooks/`; the bootstrap and molecule playbooks stay with their tooling (`bootstrap/`, `molecule/<tier>/`). Operator runbooks live in `docs/` (e.g. disaster recovery). OpenTofu config for cloud infrastructure lives in a sibling `terraform/` tree, with remote state in a GCS bucket in the infra-shared GCP project; a PR plans it and a merge to main auto-applies it to live cloud infra via CI (keyless WIF) — see `terraform/README.md`. The `jonnyoc.uk` website is a Hugo static site in `jonnyoc-site/`, built and deployed to Firebase Hosting by CI — see `jonnyoc-site/README.md`. `packer/` builds the openSUSE MicroOS image rogue-trader runs, in two stages against real Hetzner servers; the build is operator-run, never CI-run, and only its `fmt`/`validate` gates run in CI — see `packer/README.md`.
...
Every role must ship a `default` or `libvirt` scenario (or both), and a `libvirt` scenario requires a `hetzner` one. `bin/check-role-test-coverage.sh` (a pre-commit hook) enforces this. Prefer incus — add the full-VM tier only when a container can't test the role. `motd` is the exception: it carries every tier as the harness exemplar.

Shared, role-agnostic create and destroy playbooks live in `molecule/<tier>/`, where `<tier>` is `incus`, `libvirt`, or `hetzner` (the incus tier also has a shared `prepare.yml`); the container tier ships one scenario, `default` (Tumbleweed), built from the `incus` playbooks. Everything about a scenario that its tier decides — those playbook paths, `roles_path`, the incus connection, hetzner's SSH transfer method — lives in that tier's `molecule/<tier>/base.yml`, which molecule deep-merges under the scenario. So a `molecule.yml` holds only its `platforms:` (naming instances `lex-<role>-<token>-${MOLECULE_RUN_ID:-local}` — the token is the scenario name, except `default`'s is `incus`; underscores in the role name hyphenated — so concurrent runs never collide), plus any override it layers on top. A tier's config reaches molecule only through `-c`, which the make targets pass: **drive scenarios through the make targets, never bare `molecule`** — a bare run is trapped, and `molecule/README.md` says why. converge/verify are role-specific and live in the role's primary scenario (`default`, or `libvirt` where there is no container tier); the other scenarios symlink them, so a role keeps one of each. CI tests the roles a PR changes plus any role that consumes one of them through `include_role` — an engine role is exercised by its consumers' scenarios, not its own — plus `motd` for shared-infra changes; though a `requirements-dev.txt`- or `Makefile`-only (toolchain) bump runs the free incus tier only, and a docs-only (`*.md`) change runs no tier at all.
...
- **Setup**: `python -m venv .venv && . .venv/bin/activate && pip install -r requirements-dev.txt`, then `make hooks` to install the pre-commit hooks. The `terraform/` gates additionally need `tofu` and `tflint` on PATH — the `dev` role provisions both on the workstation; see `terraform/README.md` to install them elsewhere. The alert-rules gate needs `promtool` on PATH too, which the `dev` role provisions from zypper; see `roles/dev/README.md`. The `packer/` gates need HashiCorp `packer` on PATH, which the `dev` role deliberately does not install, being needed only when `packer/` changes — install it by hand, at the version `.github/workflows/lint.yml` pins if you intend to build an image, and set `PACKER` if the real binary is not first on PATH (openSUSE's cracklib owns the name); see `packer/README.md`. The `*.bu` gate needs `butane`, which the `dev` role does provide.
