...
- Keep sensitive topology out of the repo — public IPs, exposed ports, VPN/internal-network layout, and anything else that maps the attack surface. Two exceptions, both deliberate: apex domains, which must live in terraform and caddy, so they are not treated as secret; and `README.md`'s Network section and stack prose as they stand — the FTTP uplink and the gateway, the switch and AP names with their room labels, every link speed, which device hangs off which switch, that `rogue-trader` joins the fleet over WireGuard and sits behind Cloudflare's edge, that `administratum` sources both off-site copies (rsync through `rogue-trader`'s proxy to `port-wander` beside it in Hetzner, S3 direct to `reclusiam` at Cloudflare), and the four flows out of the GitHub repo (arbites into Home, molecule to the Hetzner CI VMs, the Firebase deploy to the site on Google Cloud, tofu to its providers). Neither is licence for more: never add addresses, VLANs, ports or firewall posture, never a VPN parameter beyond that protocol name, and never extend the diagrams or that prose past what they already show. Private addresses are not that surface: an RFC1918 literal may stay where the code needs it and the line beside it says why — the tunnel address in `inventory/hosts.yml`, the NAS and storage-box literals in `playbooks/rogue-trader.yml` — and those are the whole of it, not a warrant for another.
